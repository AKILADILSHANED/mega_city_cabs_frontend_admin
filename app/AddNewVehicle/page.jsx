...
export default function AddVehicle() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      newErrorState.vehicleNumber ||
      newErrorState.vehicleModel ||
      newErrorState.vehicleType
    ) {
      return;
    }
    try {
      //Create API request to send the frontend data to the backend..
      const request = await fetch(
        "http://localhost:8080/api/v1/vehicle/vehicle-register",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            vehicleNumber: vehicleNumnerState,
      ...
      <form onSubmit={handleSubmit}>
        <div className="mt-4 border-none shadow-md hover:shadow-lg h-[250px] min-w-screen flex-col items-start justify-start">
          <div className="mb-4">
            ...
            <input
              onChange={(e) => setVehicleNumberState(e.target.value)}
              value={vehicleNumnerState}
              className="text-md w-[300px] ring-1 ring-blue-500 ml-2 border-none rounded-sm hover:shadow-md px-2"
              type="text"
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className="text-md w-[300px] ring-1 ring-blue-500 ml-7 border-none rounded-sm hover:shadow-md px-2"
              type="text"
              placeholder="Enter Vehicle Type (Ex: Car, Lorry)"
            />
            {error.vehicleType && (
              <div>
                <label className="text-red-400 ml-[140px] text-sm">
          ...
          <div className="mb-4">
            ...
            <input
              onChange={(e) => setVehicleModelState(e.target.value)}
              className="text-md w-[300px] ring-1 ring-blue-500 ml-5 border-none rounded-sm hover:shadow-md px-2"
              type="text"
